...
```
.
├── api/                    # OpenAPI spec (regenerated by make generate-openapi)
...
│   ├── message_queue/
│   └── streamer/
├── docs/                   # Architecture docs + DCGM CSV data file
...
The full OpenAPI 3.0 spec is at [`api/openapi.yaml`](api/openapi.yaml).  
Regenerate it at any time with `make generate-openapi`.
...
```bash
curl http://localhost:9090/api/v1/gpus
```
...
curl "http://localhost:9090/api/v1/gpus/${GPU_ID}/telemetry"

# Specific time window
curl "http://localhost:9090/api/v1/gpus/${GPU_ID}/telemetry?start_time=2025-07-18T20:00:00Z&end_time=2025-07-18T21:00:00Z"
```
...
curl -s http://localhost:9090/api/v1/gpus | python3 -m json.tool

# Step 4: Copy a GPU UUID from the response and query its telemetry
GPU_ID="GPU-5fd4f087-86f3-7a43-b711-4771313afc50"
curl -s "http://localhost:9090/api/v1/gpus/${GPU_ID}/telemetry" | python3 -m json.tool

# Step 5: Query a specific time window
curl -s "http://localhost:9090/api/v1/gpus/${GPU_ID}/telemetry?start_time=2025-07-18T20:00:00Z&end_time=2025-07-18T21:00:00Z" \
  | python3 -m json.tool
...
The spec at `api/openapi.yaml` is generated from `@Summary`, `@Param`, `@Router`, and `@Success` annotations embedded in:
...
This installs [swag](https://github.com/swaggo/swag) on demand (via `go install`) then runs:
