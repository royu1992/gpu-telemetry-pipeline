...
import (
	...
	_ "github.com/royu1992/gpu-telemetry-pipeline/docs/api/api-gateway"
...
//	GET /gpus                          – list all distinct GPUs (cached)
//	GET /gpus/:id/telemetry            – time-series data for a single GPU
func (h *Handler) RegisterRoutes(r *gin.Engine) {
	// Probes and observability are served at the root level.
	r.GET("/health", h.handleHealth)
	r.GET("/ready", h.handleReady)
	r.GET("/metrics", h.handleMetrics)

	// Swagger UI documentation endpoint.
	// Serves the OpenAPI 3.0 specification generated by 'make generate-openapi'.
	r.GET("/docs/*any", ginSwagger.WrapHandler(swaggerFiles.Handler,
		ginSwagger.InstanceName("openapi_api_gateway")))

	// All public API routes.
	r.GET("/gpus", h.handleListGPUs)
	r.GET("/gpus/:id/telemetry", h.handleGetTelemetry)
}
...
func (h *Handler) handleMetrics(c *gin.Context) {
	// Snapshot returns a consistent struct for serialisation; no error path expected.
...
// @Success      200  {object}  TelemetryResponse  "Telemetry response with id, count, and data array"
// @Failure      400  {object}  ErrorResponse      "Bad request – invalid time format or start > end"
...
func (h *Handler) handleGetTelemetry(c *gin.Context) {
	...
	if uuid == "" {
		h.metrics.IncRequestsError()
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "missing GPU id"})
		return
	}
	...
	if err != nil {
		h.metrics.IncRequestsError()
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: fmt.Sprintf("invalid start_time: %s", err)})
		return
	}
	...
	if err != nil {
		h.metrics.IncRequestsError()
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: fmt.Sprintf("invalid end_time: %s", err)})
		return
	}
	...
	if startTime.After(endTime) {
		h.metrics.IncRequestsError()
		c.JSON(http.StatusBadRequest, ErrorResponse{Error: "start_time must not be after end_time"})
		return
	}
	...
	if !exists {
		...
		h.metrics.IncRequestsError()
		c.JSON(http.StatusNotFound, ErrorResponse{Error: fmt.Sprintf("GPU %q not found", uuid)})
		return
	}
...
func (h *Handler) respondError(c *gin.Context, err error) {
	...
	if errors.Is(err, context.DeadlineExceeded) {
		c.JSON(http.StatusGatewayTimeout, ErrorResponse{Error: "database query timed out"})
		return
	}
	...
	if errors.Is(err, context.Canceled) {
		c.JSON(http.StatusServiceUnavailable, ErrorResponse{Error: "request cancelled"})
		return
	}
